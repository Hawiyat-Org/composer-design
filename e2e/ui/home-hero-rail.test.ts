  ...
  },
  {
    ...
    manifest: {
      ...
      od: {
        ...
        useCase: {
          query:
            'Create refreshable, auditable Composer Design artifacts backed by connector or local data.',
        },
      },
    },
  },
...
async function waitForLoadingToClear(page: Page) {
  await expect(page.getByText('Loading Composer Design…')).toHaveCount(0, { timeout: 15_000 });
}
...
test('[P1] selecting another example updates the composer input', async ({ page }) => {
  ...
  await page
    ...
    .click();
  await expect(input).toHaveText('Create refreshable, auditable Composer Design artifacts backed by connector or local data.');
});
