...
test('[P0] after local Sign out, the app returns to Cloud sign-in without clearing setup', async ({ page }) => {
  ...
  // "ComposerDesign MCP" nav item, so a surface-wide /ComposerDesign/i now resolves
  ...
  await expect(page.getByRole('button', { name: /Sign in to ComposerDesign|登录 ComposerDesign/i })).toBeVisible();
