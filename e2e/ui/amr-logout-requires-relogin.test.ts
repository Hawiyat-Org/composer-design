...
test('[P0] after local Sign out, AMR runs require re-login and Settings keeps AMR selected', async ({ page }) => {
  ...
  await expect(settings.getByRole('button', { name: /Composer Design AMR/i }).first()).toHaveAttribute('aria-pressed', 'true');
  ...
  await expect(reopenedSettings.getByRole('button', { name: /Composer Design AMR/i }).first()).toHaveAttribute('aria-pressed', 'true');
