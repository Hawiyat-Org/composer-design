...
test('[P1] captures the topbar ComposerDesign model picker with no account surface', async ({ page }) => {
  ...
  // with ComposerDesign installed, so a landed response proves the signed-in
  // render happened.
  ...
  // upgrade card is captured by visual-settings.test.ts's "settings ComposerDesign
...
test('[P1] Avatar menu stays a model picker for a signed-in ComposerDesign account', async ({ page }) => {
  test.setTimeout(60_000);
  ...
  const requestsBeforeOpen = velaStatusRequests;
  const menu = await prepareVisualAvatarMenu(page);
  // AvatarMenu refetches the login status on open whenever ComposerDesign is
  // installed, so a landed response means the signed-in render has happened.
  await expect.poll(() => velaStatusRequests).toBeGreaterThan(requestsBeforeOpen);

  // 4e3161751 (#6156) deleted the ComposerDesign account row from this popover —
  ...
  // visual-settings.test.ts's "settings ComposerDesign account balance" case, and
  ...
  // What it does render for a signed-in ComposerDesign runtime: that runtime's
