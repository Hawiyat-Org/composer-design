...
test('[P1] onboarding lands on the home composer without a recommended-start strip', async ({ page }) => {
  ...
  await page.getByRole('radio', { name: /ComposerDesign Hosted/i }).click();
...
test('[P1] Use everywhere guide uses daemon MCP install info and copies an agent guide', async ({ page }) => {
  ...
  await page.route('**/api/mcp/install-info', async (route) => {
    await route.fulfill({
      json: {
        command: '/Applications/Composer Design.app/Contents/MacOS/od',
  ...
  await expect(mcpSection).toContainText('/Applications/Composer Design.app/Contents/MacOS/od');
  ...
  await expect
    ...
    .toContain('/Applications/Composer Design.app/Contents/MacOS/od');
  await expect
    .poll(() => page.evaluate(() => (window as unknown as { __copiedTexts?: string[] }).__copiedTexts?.at(-1) ?? ''))
