...
async function openNewProjectFromEmptyProjects(page: Page): Promise<void> {
  ...
  await expect(page.getByText('Loading ComposerDesign…')).toHaveCount(0, { timeout: 15_000 });
...
test('[P0] UI-created Personal project recovers preview and write authority after reload', async ({ page }) => {
  ...
  try {
    ...
    // (`Loading ComposerDesign…`) and the project-route workspace-context gate
    ...
    await page
      .getByText('Loading ComposerDesign…')
...
async function expectWorkspaceReady(page: Page) {
  await expect(page).toHaveURL(/\/projects\//);
  await page.getByText('Loading ComposerDesign…').waitFor({ state: 'hidden', timeout: T.long }).catch(() => {});
...
async function dismissPrivacyDialog(page: Page) {
  const privacyRegion = page.getByRole('region', { name: /Help us improve ComposerDesign/i });
