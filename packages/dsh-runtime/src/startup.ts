...
export const OPEN_DESIGN_STARTUP_SERVICE = 'composerDesignStartup';

export interface ComposerDesignStartupValues {
  mode: 'models' | 'probe' | 'stdio';
}

declare module '@deepseek-ai/cordis' {
  interface Context {
    composerDesignStartup?: ComposerDesignStartupValues;
  }
}
...
export function apply(ctx: Context): void {
  const program = new Command()
    ...
    .description('Run the ComposerDesign JSONL profile adapter.')
    .helpOption('-h, --help', 'show this help')
    .option('--models', 'print the Harness model catalog and exit')
    .option('--probe', 'print profile compatibility and exit')
    .option('--stdio', 'serve one ComposerDesign run over JSONL stdio')
    .action((options: { models?: boolean; probe?: boolean; stdio?: boolean }) => {
      ...
      let mode: ComposerDesignStartupValues['mode'] = 'stdio';
