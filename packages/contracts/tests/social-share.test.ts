...
describe('social-share contract', () => {
  it('builds ComposerDesign repository share targets', () => {
    const payload = buildSocialSharePayload({
      kind: 'open-design-repo',
      locale: 'zh-CN',
      title: 'ComposerDesign GitHub',
      text: '推荐 ComposerDesign',
    });
  ...
  it('keeps deployed project links and the repo recommendation together', () => {
    const payload = buildSocialSharePayload({
      ...
      title: 'Demo',
      text: `Built with ComposerDesign. Repo: ${OPEN_DESIGN_GITHUB_REPO_URL}`,
