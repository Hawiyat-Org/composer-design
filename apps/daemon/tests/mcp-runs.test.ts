...
describe('public MCP discovery + generation tools', () => {
  ...
  it('get_run returns a recharge link and same-request resume instructions', async () => {
    ...
    expect(parsed).toMatchObject({
      status: 'failed',
      failureAction: 'recharge',
      rechargeUrl: 'https://open-design.ai/amr/dashboard?source=composer_design',
    });
  ...
  // Discovery-stage / clarifying-question fallback: when ComposerDesign's
  ...
  // #2: MCP-driven projects skip ComposerDesign's interactive discovery
