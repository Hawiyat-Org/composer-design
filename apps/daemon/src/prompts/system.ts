...
This run uses Composer Design's filesystem execution profile. Project files are the source of truth for generated artifacts.
...
export function composeSystemPrompt({
  ...
  if (agentId === 'gemini') {
    parts.push(
      "\n\n---\n\n## Gemini todo tool mapping\n\nWhen a Composer Design instruction says to call `TodoWrite`, use Gemini CLI's native `write_todos` tool only if it is present in the current tool list. Pass the full task list as `todos`, with each item using `description` for the task text and `status` set to `pending`, `in_progress`, `completed`, `cancelled`, or `blocked`.\n\nIf `write_todos` is not present, do not simulate it with markdown, plan-mode files, JSON files, TODO files, or shell commands. Continue the work normally without a todo tool.",
    );
  }
...
function renderMediaMetadataAction(
  ...
  const mode = mediaExecution?.mode ?? 'enabled';
  if (mode === 'disabled') {
    return `This is ${article} **${surface}** project, but Composer Design-owned media execution is disabled for this run. Plan the creative brief only unless an external MCP media tool is explicitly configured. Do NOT call OD media generation tools and do NOT emit \`<artifact>\` HTML for media surfaces.`;
  }
