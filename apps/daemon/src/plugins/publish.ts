...
//   - composer-design            → nexu-io/composer-design (plugins/community/<plugin-name>/).
...
export function buildPublishLink(args: {
  ...
  switch (args.catalog) {
    ...
    case 'open-design': {
      ...
      const url = newIssueUrl('nexu-io/composer-design', title, bodyWithRegistry);
      return {
        catalog: args.catalog,
        catalogLabel: 'nexu-io/composer-design',
        url,
...
function renderPrBody(m: PublishMetadata): string {
  ...
  lines.push('- Ships `open-design.json` sidecar (additive ComposerDesign metadata).');
  lines.push('');
