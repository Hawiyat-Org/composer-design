...
//   - open-design            → hawiyat/composer-design (plugins/community/<plugin-name>/).
  ...
  switch (args.catalog) {
    ...
    case 'open-design': {
      const bodyWithRegistry = [
        body,
        '',
        '## Composer Design registry entry',
        '',
      ...
      const url = newIssueUrl('hawiyat/composer-design', title, bodyWithRegistry);
      return {
        catalog: args.catalog,
        catalogLabel: 'hawiyat/composer-design',
        url,
...
function renderPrBody(m: PublishMetadata): string {
  ...
  lines.push('- Ships `open-design.json` sidecar (additive Composer Design metadata).');
  lines.push('');
