...
// (Claude Code, Cursor, Zed) pull files from a local Composer Design
...
const PROJECT_ARG = {
  type: 'string',
  description: 'Project id (UUID) or name substring. Optional; defaults to the active project (expires after ~5 minutes of no Composer Design activity).',
} as const;

const TOOL_DEFS = [
  {
    name: 'list_projects',
    description: 'List every Composer Design project on this daemon.',
    inputSchema: { type: 'object', properties: {}, additionalProperties: false },
    annotations: { ...READ_ANNOTATIONS, title: 'List Composer Design projects' },
  },
  {
    name: 'get_active_context',
    description:
      'Project + file the user has open in Composer Design right now. Returns {active:false, hint:"..."} when no project is active so the agent can ask the user to interact with Composer Design (the active context expires ~5 minutes after the last user interaction). Most tools default to this when project is omitted, so you rarely need to call this directly.',
    inputSchema: { type: 'object', properties: {}, additionalProperties: false },
    ...
    inputSchema: {
      ...
      properties: {
        ...
        entry: {
          type: 'string',
          description:
            "Entry file path relative to project root. Defaults to the active file or project's metadata.entryFile. Active-file fallback expires after ~5 minutes of no Composer Design activity.",
        },
        include: {
          type: 'string',
  ...
  },
  {
    ...
    annotations: { ...READ_ANNOTATIONS, title: 'Get Composer Design project' },
  },
  {
    name: 'get_file',
    description:
    ...
    inputSchema: {
      ...
      properties: {
        ...
        path: {
          type: 'string',
          description:
            'File path relative to project root, forward slashes. Optional; defaults to the active file when project is also omitted. Active-file fallback expires after ~5 minutes of no Composer Design activity.',
        },
        offset: {
          type: 'number',
  ...
  },
  {
    ...
    description:
      'Create one normal Composer Design project artifact entry file. Writes name+content, rejects existing targets, and persists artifactManifest when supplied. HTML, Markdown, and SVG entries get a default manifest when omitted. Project optional; defaults to the active project.',
    inputSchema: {
      type: 'object',
      properties: {
        project: PROJECT_ARG,
        ...
        artifactManifest: {
          ...
          description: 'Optional ArtifactManifest sidecar. If omitted, Composer Design infers one for HTML, Markdown, or SVG entry files.',
        },
      },
      required: ['name', 'content'],
      additionalProperties: false,
    },
    annotations: { ...WRITE_ANNOTATIONS, title: 'Create Composer Design artifact' },
  },
  {
    name: 'write_file',
    description:
    ...
    annotations: { ...WRITE_ANNOTATIONS, title: 'Write Composer Design project file' },
  },
  {
    name: 'delete_file',
    description:
    ...
    annotations: { ...WRITE_ANNOTATIONS, destructiveHint: true, title: 'Delete Composer Design project file' },
  },
  {
    name: 'delete_project',
    description:
      'Permanently delete a Composer Design project including its files and conversations. Requires both an explicit project id/name AND confirm:true — there is no active-project fallback because the operation is irreversible.',
    inputSchema: {
      type: 'object',
      properties: {
        project: {
    ...
    annotations: { ...WRITE_ANNOTATIONS, destructiveHint: true, title: 'Delete Composer Design project' },
  },
  {
    name: 'create_project',
    description:
      'Create a new empty Composer Design project to generate into, then call start_run against it. Returns the project (with its id) plus a conversationId. The id is derived from name unless you pass one explicitly.',
    inputSchema: {
      type: 'object',
      properties: {
    ...
    annotations: { ...WRITE_ANNOTATIONS, title: 'Create Composer Design project' },
  },
  // Discovery + generation. An external coding agent does NOT run a
  // skill itself — it commissions Composer Design to, via start_run. The
  ...
  {
    name: 'list_skills',
    description: 'List Composer Design skills you can pass to start_run as a recipe. Discovery only — Composer Design runs the skill, not you.',
    inputSchema: { type: 'object', properties: {}, additionalProperties: false },
    annotations: { ...READ_ANNOTATIONS, title: 'List Composer Design skills' },
  },
  {
    name: 'list_plugins',
    description: 'List installed Composer Design plugins (packaged design workflows) you can pass to start_run as plugin + inputs.',
    inputSchema: { type: 'object', properties: {}, additionalProperties: false },
    annotations: { ...READ_ANNOTATIONS, title: 'List Composer Design plugins' },
  },
  {
    name: 'start_run',
    description:
      'Commission Composer Design to generate or refine a design. Composer Design spawns its own agent to do the work and returns a runId immediately. Poll get_run(runId) until status is terminal, then get_artifact to pull the result. Project optional; defaults to the active project. Requires an existing project (create one first with create_project).',
    inputSchema: {
      type: 'object',
      properties: {
        project: PROJECT_ARG,
        ...
        agent: {
          type: 'string',
          description: "Which agent Composer Design should run, e.g. 'claude' | 'codex' | 'gemini'. Optional; defaults to the user's configured agent.",
        },
        model: {
          type: 'string',
    ...
    annotations: { ...WRITE_ANNOTATIONS, title: 'Generate with Composer Design' },
  },
  {
    name: 'get_run',
    description:
    ...
    annotations: { ...READ_ANNOTATIONS, title: 'Check Composer Design run' },
  },
  {
    name: 'cancel_run',
  ...
  },
  {
    ...
    annotations: { ...READ_ANNOTATIONS, title: 'List Composer Design agents' },
  },
];
...
export async function runMcpStdio({ daemonUrl }: RunMcpOptions): Promise<void> {
  ...
  const server = new Server(
    { name: SERVER_NAME, version: SERVER_VERSION },
    {
      ...
      instructions: [
        'Composer Design (OD) is a local-first design workspace. The user typically',
        ...
        '',
        'To make Composer Design GENERATE or refine a design (rather than just',
        ...
        '    Composer Design spawns its own agent to do the work.',
        ...
        '',
        'Generation patience: Composer Design runs typically take 5–30',
        ...
        'deliverables — Composer Design natively produces browser-viewable',
        'HTML/SVG (including HTML-rendered decks), but the user may want a',
        'real binary file (.pptx / .docx / .pdf) which Composer Design does NOT',
        ...
        '',
        'When extending a Composer Design design in another codebase, pull',
  ...
  server.setRequestHandler(ListResourcesRequestSchema, withMcpActivity(async () => {
    ...
    const resources = [
      {
        uri: 'od://focus/active',
        name: 'Active Composer Design context',
        description: 'The project/file the user has open in Composer Design right now.',
    ...
    switch (name) {
      ...
      case 'get_active_context': {
        ...
        if (!data || data.active === false) {
          return ok({
            active: false,
            hint: 'Composer Design has no active project right now. The active context expires about 5 minutes after the last user interaction with Composer Design, so the user may need to click into a project (or switch tabs inside one) to wake it up. Alternatively, pass project="<id-or-name>" to other tools to bypass active context entirely.',
          });
        }
        return ok(data);
      }
...
// the user had already created in Composer Design.
//
...
async function getRun(baseUrl: string, args: McpArgs) {
  ...
  if (status.status !== 'succeeded' || typeof status.projectId !== 'string' || !status.projectId) {
    ...
    if (typeof status.eventsLogPath === 'string' && status.eventsLogPath.length > 0) {
      ...
      if (studioUrl) {
        enriched.hint += ` Once you have something to show the user, give them a clickable markdown link to studioUrl — render it as \`[Watch progress in Composer Design studio](${studioUrl})\`, NEVER as inline code or bare text, so clients like Codex / Cursor / Zed make it navigable in their built-in browser pane.`;
      }
    }
    return ok(enriched);
  }
...
// host-injected React/Babel do not — those still need the Composer Design
...
// renamed in the Composer Design UI shows up within a few seconds.
...
// open in Composer Design. Returns the resolved id plus, for echo-back to the
...
async function resolveProjectArg(baseUrl: string, arg: unknown): Promise<{ id: string; resolved: ResolvedProject | null; active: ActiveContext | null }> {
  ...
  if (!active || active.active === false || !active.projectId) {
    throw new Error(
      'project arg omitted and Composer Design has no active project. The active context expires about 5 minutes after the last user interaction with Composer Design - the user may need to click into a project to wake it up. Otherwise pass project="<id-or-name>".',
    );
  }
...
function formatError(err: unknown, daemonUrl: string): string {
  ...
  if (code === 'ECONNREFUSED' || code === 'ENOTFOUND') {
    return `cannot reach the Composer Design daemon at ${daemonUrl}. Is it running? Start it with \`pnpm tools-dev\`.`;
  }
  return msg;
}
