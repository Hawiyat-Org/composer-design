...
// (Claude Code, Cursor, Zed) pull files from a local ComposerDesign
...
export const MCP_SERVER_INSTRUCTIONS = [
  'Use only these product names in user-facing replies: ComposerDesign Cloud and Local Codex.',
...
function isDaemonUnreachableResult(result: McpToolCallResult): boolean {
  return result.isError === true
    && result.content.some((item) =>
      item.text.includes('cannot reach the ComposerDesign daemon'),
    );
}
...
const PROJECT_ARG = {
  type: 'string',
  description: 'Project id (UUID) or name substring. Optional; defaults to the active project (expires after ~5 minutes of no ComposerDesign activity).',
} as const;

const PLUGIN_WORKFLOW_ID_ARG = {
  type: 'string',
  description:
    'Opaque workflow id issued by the local ComposerDesign MCP after the first attributed call. Reuse it for later plugin-attributed calls; never invent or display it.',
} as const;
...
const EXTERNAL_PLUGIN_CONTEXT_ARG = {
  ...
  properties: {
    ...
    publisherClass: {
      type: 'string',
      enum: ['composer_design_first_party', 'third_party', 'unknown'],
    },
  },
  required: [
    'id',
...
export const TOOL_DEFS = [
  {
    name: 'collect_brief',
    description:
      'Open an interactive ComposerDesign brief card for a new artifact. Use the returned human-readable confirmation with any explicit execution mode; never ask the user to copy an internal draft id or nonce.',
    inputSchema: {
      type: 'object',
      properties: {
        artifactType: {
    ...
    annotations: { ...WRITE_ANNOTATIONS, title: 'Collect ComposerDesign brief' },
    _meta: {
  ...
  },
  {
    name: 'confirm_brief',
    description:
      'Confirm the choices from the rendered ComposerDesign brief card. Returns a readable summary; draft ids and nonces are internal widget data, never user-facing copy.',
    inputSchema: {
      type: 'object',
      properties: {
        briefDraftId: {
    ...
    annotations: { ...WRITE_ANNOTATIONS, title: 'Confirm ComposerDesign brief' },
  },
  {
    name: 'list_projects',
    description: 'List every ComposerDesign project on this daemon.',
    inputSchema: {
      type: 'object',
      properties: { pluginWorkflowId: PLUGIN_WORKFLOW_ID_ARG },
      additionalProperties: false,
    },
    annotations: { ...READ_ANNOTATIONS, title: 'List ComposerDesign projects' },
  },
  {
    name: 'get_active_context',
    description:
      'Project + file the user has open in ComposerDesign right now. Returns {active:false, hint:"..."} when no project is active so the agent can ask the user to interact with ComposerDesign (the active context expires ~5 minutes after the last user interaction). Most tools default to this when project is omitted, so you rarely need to call this directly.',
    inputSchema: {
      type: 'object',
      properties: { pluginWorkflowId: PLUGIN_WORKFLOW_ID_ARG },
      additionalProperties: false,
    ...
    inputSchema: {
      ...
      properties: {
        ...
        entry: {
          type: 'string',
          description:
            "Entry file path relative to project root. Defaults to the active file or project's metadata.entryFile. Active-file fallback expires after ~5 minutes of no ComposerDesign activity.",
        },
        include: {
          type: 'string',
  ...
  },
  {
    ...
    annotations: { ...READ_ANNOTATIONS, title: 'Get ComposerDesign project' },
  },
  {
    name: 'get_file',
    description:
    ...
    inputSchema: {
      ...
      properties: {
        ...
        path: {
          type: 'string',
          description:
            'File path relative to project root, forward slashes. Optional; defaults to the active file when project is also omitted. Active-file fallback expires after ~5 minutes of no ComposerDesign activity.',
        },
        offset: {
          type: 'number',
  ...
  },
  {
    ...
    description:
      'Create one normal ComposerDesign project artifact entry file. Writes name+content, rejects existing targets, and persists artifactManifest when supplied. HTML, Markdown, and SVG entries get a default manifest when omitted. Project optional; defaults to the active project.',
    inputSchema: {
      type: 'object',
      properties: {
        project: PROJECT_ARG,
        ...
        artifactManifest: {
          type: 'object',
          additionalProperties: true,
          description: 'Optional ArtifactManifest sidecar. If omitted, ComposerDesign infers one for HTML, Markdown, or SVG entry files.',
        },
      },
      required: ['name', 'content'],
      additionalProperties: false,
    },
    annotations: { ...WRITE_ANNOTATIONS, title: 'Create ComposerDesign artifact' },
  },
  {
    name: 'write_file',
    description:
    ...
    annotations: { ...WRITE_ANNOTATIONS, title: 'Write ComposerDesign project file' },
  },
  {
    name: 'delete_file',
    description:
    ...
    annotations: { ...WRITE_ANNOTATIONS, destructiveHint: true, title: 'Delete ComposerDesign project file' },
  },
  {
    name: 'delete_project',
    description:
      'Permanently delete an ComposerDesign project including its files and conversations. Requires both an explicit project id/name AND confirm:true — there is no active-project fallback because the operation is irreversible.',
    inputSchema: {
      type: 'object',
      properties: {
        project: {
    ...
    annotations: { ...WRITE_ANNOTATIONS, destructiveHint: true, title: 'Delete ComposerDesign project' },
  },
  {
    name: 'create_project',
    description:
      'Create a new empty ComposerDesign project to generate into, then call start_run against it. Returns the project (with its id) plus a conversationId. The id is derived from name unless you pass one explicitly.',
    inputSchema: {
      type: 'object',
      properties: {
    ...
    annotations: { ...WRITE_ANNOTATIONS, title: 'Create ComposerDesign project' },
  },
  // Discovery + generation. An external coding agent does NOT run a
  // skill itself — it commissions ComposerDesign to, via start_run. The
  ...
  {
    name: 'list_skills',
    description: 'List ComposerDesign skills you can pass to start_run as a recipe. Discovery only — ComposerDesign runs the skill, not you.',
    inputSchema: {
      type: 'object',
      properties: { pluginWorkflowId: PLUGIN_WORKFLOW_ID_ARG },
      additionalProperties: false,
    },
    annotations: { ...READ_ANNOTATIONS, title: 'List ComposerDesign skills' },
  },
  {
    name: 'list_plugins',
    description: 'List installed ComposerDesign plugins (packaged design workflows) you can pass to start_run as plugin + inputs.',
    inputSchema: {
      type: 'object',
      properties: { pluginWorkflowId: PLUGIN_WORKFLOW_ID_ARG },
      additionalProperties: false,
    },
    annotations: { ...READ_ANNOTATIONS, title: 'List ComposerDesign plugins' },
  },
  {
    name: 'start_vela_login',
    description:
      'Start ComposerDesign Cloud browser sign-in through the local ComposerDesign daemon. Returns the activation URL and user code when manual browser completion is needed. The tool name is an internal compatibility identifier and must not be repeated to the user.',
    inputSchema: {
      type: 'object',
      properties: { pluginWorkflowId: PLUGIN_WORKFLOW_ID_ARG },
      additionalProperties: false,
    ...
    annotations: {
      ...WRITE_ANNOTATIONS,
      openWorldHint: true,
      title: 'Sign in to ComposerDesign Cloud',
    },
  },
  {
    name: 'get_vela_login_status',
    description:
      'Check whether ComposerDesign Cloud browser sign-in is complete. Does not expose credentials. The tool name is an internal compatibility identifier and must not be repeated to the user.',
    inputSchema: {
      type: 'object',
      properties: { pluginWorkflowId: PLUGIN_WORKFLOW_ID_ARG },
      additionalProperties: false,
    ...
    annotations: {
      ...READ_ANNOTATIONS,
      openWorldHint: true,
      title: 'Check ComposerDesign Cloud sign-in',
    },
  },
  {
    name: 'start_run',
    description:
      'Commission ComposerDesign to generate or refine a design. ComposerDesign spawns its own agent to do the work and returns a runId immediately. Poll get_run(runId) until status is terminal; its Preview/Studio reference is the default delivery. Call get_artifact only when source context is genuinely needed. Project optional; defaults to the active project. Requires an existing project (create one first with create_project).',
    inputSchema: {
      type: 'object',
      properties: {
        project: PROJECT_ARG,
        ...
        resume: {
          type: 'boolean',
          description:
            'Set true only after the user has topped up a paused ComposerDesign Cloud run. Reuse the exact original requestId and payload; ComposerDesign resumes the same logical run.',
        },
        pluginWorkflowId: PLUGIN_WORKFLOW_ID_ARG,
      },
      additionalProperties: false,
    },
    annotations: { ...WRITE_ANNOTATIONS, title: 'Generate with ComposerDesign' },
  },
  {
    name: 'get_run',
    description:
    ...
    annotations: { ...READ_ANNOTATIONS, title: 'Check ComposerDesign run' },
  },
  {
    name: 'cancel_run',
  ...
  },
  {
    ...
    annotations: { ...READ_ANNOTATIONS, title: 'List ComposerDesign agents' },
  },
];
  ...
  return [
    {
      ...
      name: 'ComposerDesign brief',
      title: 'Choose the artifact direction',
      description:
        'Interactive local ComposerDesign brief card shared by ComposerDesign Cloud and Local Codex modes.',
...
export async function _listMcpResources(
  ...
  const resources = [
    ...
    {
      uri: 'od://focus/active',
      name: 'Active ComposerDesign context',
      description: 'The project/file the user has open in ComposerDesign right now.',
...
function mcpFailureFacts(
  ...
  const errorCode = message.includes('PLUGIN_CONTRACT_REJECTED')
    ...
    : message.includes('cannot reach the ComposerDesign daemon')
      ? 'DAEMON_UNREACHABLE'
  ...
  const server = new Server(
    { name: SERVER_NAME, version: SERVER_VERSION },
    {
      ...
      instructions: [
        ...
        '',
        'ComposerDesign (OD) is a local-first design workspace. The user typically',
        ...
        '',
        'To make ComposerDesign GENERATE or refine a design (rather than just',
        ...
        '    confirm_brief returns the readable brief to reuse with ComposerDesign',
        ...
        ' - for ComposerDesign Cloud, call the Cloud login-status tool first.',
        ...
        '    ComposerDesign spawns its own agent to do the work.',
        ...
        '',
        'Generation patience: ComposerDesign runs typically take 5–30',
        ...
        'deliverables — ComposerDesign natively produces browser-viewable',
        'HTML/SVG (including HTML-rendered decks), but the user may want a',
        'real binary file (.pptx / .docx / .pdf) which ComposerDesign does NOT',
        ...
        '',
        'When extending an ComposerDesign design in another codebase, pull',
    ...
    switch (name) {
      ...
      case 'get_active_context': {
        ...
        if (!data || data.active === false) {
          return ok({
            active: false,
            hint: 'ComposerDesign has no active project right now. The active context expires about 5 minutes after the last user interaction with ComposerDesign, so the user may need to click into a project (or switch tabs inside one) to wake it up. Alternatively, pass project="<id-or-name>" to other tools to bypass active context entirely.',
          });
        }
        return ok(data);
      }
...
// the user had already created in ComposerDesign.
//
...
async function startRun(
  ...
  if (
    ...
    throw new Error(
      'raw API keys are not accepted by ComposerDesign MCP. Configure Local BYOK in the ComposerDesign UI and start that run from the local product instead.',
    );
  }
  const { id, resolved, active } = await resolveProjectArg(baseUrl, args.project, headers);
    ...
    withActiveEcho(
      {
        ...
        ...(studioUrl
          ...
          : {}),
        hint: 'Run started. ComposerDesign generation normally takes 5–30 minutes. Polls showing status:running with no new files / unchanged file mtimes is the inner agent thinking, NOT a hang — DO NOT cancel_run out of impatience and DO NOT substitute write_file to produce the design yourself; OD\'s pipeline is what gives the result its design quality. Poll get_run(runId) every 30–60 seconds; report "still working" to the user between polls and keep waiting. On terminal status, artifactRef is the durable identity; previewUrl and studioUrl are browser links for the current ComposerDesign runtime and must be refreshed with get_run after ComposerDesign restarts.',
      },
      active,
      resolved,
    ),
...
async function getRun(
  ...
  if (status.status !== 'succeeded' || typeof status.projectId !== 'string' || !status.projectId) {
    ...
    if (status.failureAction === 'recharge') {
      ...
      enriched.hint =
        'ComposerDesign Cloud paused this logical run because the account balance is insufficient. Preserve the brief and project, show rechargeUrl to the user, and do not switch modes. After the user confirms the top-up, call start_run once with the exact original payload, the same requestId, and resume:true; ComposerDesign Cloud will resume the existing run and billing operation. Do not expose internal runtime or tool identifiers.';
    }
    if (typeof status.eventsLogPath === 'string' && status.eventsLogPath.length > 0) {
      ...
      if (studioUrl) {
        enriched.hint += ` While the run is in flight, studioUrl can be used as an optional workspace progress link — render it as \`[Watch progress in ComposerDesign studio](${studioUrl})\` if you choose to show it. This URL is valid for the current ComposerDesign runtime; call get_run again after ComposerDesign restarts.`;
      }
    }
    return ok(enriched);
  }
  ...
  enriched.hint = previewUrl
    ? `Run finished. artifactRef is the durable project/file identity. previewUrl and studioUrl are browser links for the current ComposerDesign runtime only; if either stops working after ComposerDesign restarts, call get_run again with this runId to obtain current links. Render previewUrl as a clickable link now. agentMessage carries the inner agent's explanation; show it alongside the link. Call get_artifact({ project: "${status.projectId}" }) when you need the source files — always pass project explicitly; omitting it falls back to the active project, which may differ. eventsLogPath, when present, holds the full inner-agent event log for forensics.`
    : 'Run finished but produced no files. The inner agent\'s output is in agentMessage — relay it to the user verbatim. Most often this is a clarifying question (e.g. a <question-form>) you should answer by calling start_run again with a more specific prompt or a chosen plugin. When studioUrl is present, show it as a clickable markdown link (`[Open ComposerDesign studio](STUDIO_URL)`) so the user can navigate to the OD page that shows the chat history — never render it as inline code. eventsLogPath, when present, holds the full event log if you need to inspect what happened.';
  return ok(enriched);
}
...
// host-injected React/Babel do not — those still need the ComposerDesign
...
// renamed in the ComposerDesign UI shows up within a few seconds.
...
// open in ComposerDesign. Returns the resolved id plus, for echo-back to the
...
async function resolveProjectArg(
  ...
  if (!active || active.active === false || !active.projectId) {
    throw new Error(
      'project arg omitted and ComposerDesign has no active project. The active context expires about 5 minutes after the last user interaction with ComposerDesign - the user may need to click into a project to wake it up. Otherwise pass project="<id-or-name>".',
    );
  }
...
function formatError(err: unknown, daemonUrl: string): string {
  ...
  if (code === 'ECONNREFUSED' || code === 'ENOTFOUND') {
    return `cannot reach the ComposerDesign daemon at ${daemonUrl}. Is it running? Start it with \`pnpm tools-dev\`.`;
  }
  return msg;
}
