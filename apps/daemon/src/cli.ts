...
// external agents (hermes-agent, openclaw, etc.) can drive Composer Design
...
function printRootHelp() {
  ...
  od plugin open-design-pr <folder>
      Push a community-catalog branch and open the Composer Design PR form.
  ...
  od share <open-design|url> [options]
      Build localized social-share targets for the Composer Design repo or a
  ...
  od mcp [--daemon-url <url>]
      ...
      running Composer Design daemon. Wire it into a coding agent
      (Claude Code, Cursor, VS Code, Zed, Windsurf) in another repo
      to pull files from a local Composer Design project and create
...
Runs Tavily-backed shallow research through the local Composer Design daemon.
  ...
  if (code === 'EPERM' || code === 'ENETUNREACH') {
    console.error(
      'hint: outbound connect was denied by a sandbox. If you launched ' +
        ...
        'network policy. The Composer Design daemon itself is unaffected - it can be ' +
...
tool calls to a running Composer Design daemon. Wire it into a coding agent
in another repo so the agent can pull files from a local Composer Design
project and create project-scoped artifacts without exporting a zip
every iteration.

Options:
  --daemon-url <url>   Composer Design daemon HTTP base URL. Resolution
...
Tools exposed:
  list_projects                  list every Composer Design project
...
user has open in Composer Design; get_artifact and get_file additionally
...
→ MCP server in the Composer Design app. The daemon must be running locally
...
Register Composer Design's stdio MCP server into a coding agent's own config.

Agents:
  ${AGENT_SLUGS.join(' ')}

Options:
  --uninstall, --remove   Remove the Composer Design MCP server instead.
...
Wraps GitHub CLI auth for Composer Design registry publishing. The token stays in gh.`);
    return;
  }
...
Shows the GitHub account gh will use for Composer Design registry publishing.`);
    return;
  }
  const host = typeof flags.host === 'string' ? flags.host : 'github.com';
...
Common options:
  --daemon-url <url>   Composer Design daemon HTTP base (default OD_DAEMON_URL, OD_SIDECAR_IPC_PATH discovery, or http://127.0.0.1:7456).
  ...
  switch (sub) {
    ...
    case 'login': {
      ...
      console.log(`[marketplace login] authenticating gh for ${host}. Tokens stay in gh, not Composer Design.`);
...
fork of hawiyat/composer-design, pushes a branch, and opens the PR form with --web.`);
    process.exit(rest.length === 0 ? 2 : 0);
  }
  const folder = rest.find((a) => !a.startsWith('-') && a !== flags.host && a !== flags.owner);
  if (!folder) {
  ...
  await run('fork', 'gh', ['repo', 'fork', 'hawiyat/composer-design'], {
...
marks a version unresolvable for new installs while preserving lockfile replay.`);
  ...
  const payload = {
    ...
    version: parsed.range,
    reason,
    url: `https://github.com/hawiyat/composer-design/issues/new?${params.toString()}`,
    body,
  };
  if (flags.json) {
    process.stdout.write(JSON.stringify(payload, null, 2) + '\n');
...
function printPluginHelp() {
  ...
  od plugin open-design-pr <folder>       Push a community-catalog branch and
                                           open the hawiyat/composer-design PR form.
...
// these so a code agent can drive Composer Design end-to-end without
...
Imports a local project directory as an editable Composer Design design system.
...
Imports a public GitHub repository as an editable Composer Design design system.
...
Imports a shadcn registry item as a Composer Design design system.
