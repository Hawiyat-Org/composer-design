...
describe('buildPackagedDaemonSpawnEnv', () => {
  ...
  it('forwards the signed packaged launcher used to bootstrap MCP headlessly', () => {
    const env = buildPackagedDaemonSpawnEnv(fakePaths(), {
      appVersion: '1.2.3',
      daemonCliEntry: '/Applications/Composer Design.app/Contents/Resources/app/prebundled/daemon/daemon-cli.mjs',
      legacyDataDir: null,
      mcpBootstrapArgs: [
        '-g',
        '-j',
        '/Applications/Composer Design.app',
        '--args',
        '--headless',
      ],
      mcpBootstrapCommand:
  ...
  it('forwards the packaged node command as OD_NODE_BIN for agent wrapper calls', () => {
    const env = buildPackagedDaemonSpawnEnv(fakePaths(), {
      ...
      daemonCliEntry: null,
      legacyDataDir: null,
      nodeCommand: 'C:\\Users\\Ada\\AppData\\Local\\Programs\\Composer Design\\resources\\open-design\\bin\\node.exe',
      requireDesktopAuth: true,
    });

    expect(env.OD_NODE_BIN).toBe(
      'C:\\Users\\Ada\\AppData\\Local\\Programs\\Composer Design\\resources\\open-design\\bin\\node.exe',
    );
  });
