...
describe("desktop updater", () => {
  ...
  it("relaunches prerelease mac launcher payloads through the prerelease app bundle", async () => {
    ...
    const launcherRuntimePath = join(root, "launcher", "runtime.json");
    const launcherRoot = root;
    const launcherLaunchPath = join(root, "installed", "Composer Design Prerelease.app");
    const launches: Array<{ appPid: number; launchPath: string; root: string }> = [];
    try {
      await mkdir(launcherLaunchPath, { recursive: true });
      await mkdir(join(root, "launcher"), { recursive: true });
      ...
      const updater = createDesktopUpdater({
        ...
        extractLauncherPayloadArchive: async ({ destinationRoot }) => {
          await mkdir(join(destinationRoot, "payload", "Composer Design Prerelease.app", "Contents", "MacOS"), { recursive: true });
          await mkdir(join(destinationRoot, "payload", "Composer Design Prerelease.app", "Contents", "Resources", "open-design"), { recursive: true });
          await writeFile(join(destinationRoot, "payload", "Composer Design Prerelease.app", "Contents", "MacOS", "Composer Design Prerelease"), "");
          await writeFile(join(destinationRoot, "payload", "Composer Design Prerelease.app", "Contents", "Resources", "open-design-config.json"), "{}\n");
          await writeFile(
            join(destinationRoot, "manifest.json"),
            `${JSON.stringify({
              channel: "prerelease",
              entry: {
                cwd: "payload/Composer Design Prerelease.app",
                executable: "payload/Composer Design Prerelease.app/Contents/MacOS/Composer Design Prerelease",
              },
              namespace: "release-prerelease",
              payloadRoot: "payload",
              platform: "darwin",
        ...
        processExecPath: join(root, "launcher", "channels", "prerelease", "namespaces", "release-prerelease", "versions", "1.0.0-prerelease.2", "payload", "Composer Design Prerelease.app", "Contents", "MacOS", "Composer Design Prerelease"),
        processPid: 4244,
      });
      ...
      const payloadLaunchPath = join(
        ...
        "1.0.0-prerelease.3",
        "payload",
        "Composer Design Prerelease.app",
        "Contents",
        "MacOS",
        "Composer Design Prerelease",
      );
