...
describe('derivePluginSourceLinks · url + local + bundled sources', () => {
  ...
  it('routes bundled official sources to the ComposerDesign repo', () => {
    const out = derivePluginSourceLinks(
      makeRecord({
        sourceKind: 'bundled',
    ...
    expect(out.sourceUrl).toBe('https://github.com/nexu-io/composer-design');
    expect(out.sourceKindLabel).toBe('Official');
    expect(out.sourceLabel).toBe('nexu-io/composer-design');
    expect(out.authorProfileUrl).toBe('https://github.com/nexu-io/composer-design');
    expect(out.homepageUrl).toBe('https://github.com/nexu-io/composer-design');
  });
});
    ...
    const out = derivePluginSourceLinks(
      makeRecord({
        manifest: {
          name:    'p',
          version: '1.0.0',
          author:  { name: 'ComposerDesign', url: 'https://github.com/nexu-io' },
        } as InstalledPluginRecord['manifest'],
      }),
    );
    expect(out.authorName).toBe('ComposerDesign');
    ...
    const out = derivePluginSourceLinks(
      makeRecord({
        ...
        manifest: {
          name:    'p',
          version: '1.0.0',
          homepage: 'https://github.com/nexu-io/composer-design',
        } as InstalledPluginRecord['manifest'],
      }),
    );
    expect(out.contributeUrl).toBe('https://github.com/nexu-io/composer-design/issues/new');
    expect(out.contributeOnGithub).toBe(true);
    expect(out.homepageUrl).toBe('https://github.com/nexu-io/composer-design');
  });
...
describe('authorInitials', () => {
  it('builds two-letter monograms', () => {
    expect(authorInitials('ComposerDesign')).toBe('OD');
