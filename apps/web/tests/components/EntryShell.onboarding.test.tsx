...
async function findCloudSignInButton() {
  return screen.findByRole('button', { name: /Sign in to ComposerDesign/i });
}
...
describe('EntryShell onboarding ComposerDesign AMR runtime', () => {
  it('gates Home on an authoritative signed-out Cloud session without clearing saved setup', async () => {
    ...
    expect(
      await screen.findByRole('heading', { name: 'Sign in to ComposerDesign' }),
    ).toBeTruthy();
  ...
  it.each([
    ...
    expect(
      screen.queryByRole('heading', { name: 'Sign in to ComposerDesign' }),
    ).toBeNull();
  });
  ...
  it('shows the model-source chooser after Cloud sign-in without exposing legacy onboarding steps', async () => {
    ...
    expect(screen.getByRole('radio', { name: /ComposerDesign Hosted/i })).toBeTruthy();
  ...
  it('supports arrow-key selection and focus within the model-source radio group', async () => {
    ...
    const hosted = await screen.findByRole('radio', { name: /ComposerDesign Hosted/i });
  ...
  it('does not auto-select ComposerDesign AMR when the AMR runtime is unavailable', async () => {
    globalThis.fetch = vi.fn(async () =>
    ...
    expect(await screen.findByRole('heading', { name: 'Sign in to ComposerDesign' })).toBeTruthy();
    expect(await findCloudSignInButton()).toBeTruthy();
    expect(screen.queryByRole('button', { name: /ComposerDesign AMR/i })).toBeNull();
  ...
  it('shows ComposerDesign Cloud as the default connect surface when AMR is available', async () => {
    globalThis.fetch = vi.fn(async () =>
      jsonResponse({ loggedIn: false, profile: 'prod', user: null, configPath: '/x' }),
    ) as typeof fetch;
    renderOnboarding();

    expect(screen.getByRole('heading', { name: 'Sign in to ComposerDesign' })).toBeTruthy();
    expect(await findCloudSignInButton()).toBeTruthy();
  ...
  it('clears AMR login pending when canceled and allows a fresh sign-in attempt', async () => {
    ...
    const cloudButton = await screen.findByRole('button', {
      name: /Sign in to ComposerDesign/i,
    });
  ...
  it('cancels AMR login and re-enables onboarding after the login timeout', async () => {
    ...
    expect(
      screen
        .getByRole('button', { name: /Sign in to ComposerDesign/i })
  ...
  it('refreshes workspace context, billing, and team projects as soon as onboarding sign-in completes', async () => {
    ...
    // in its signed-out shape (still showing "Sign in to use ComposerDesign
  ...
  it('continues normally when ComposerDesign AMR is signed in', async () => {
    globalThis.fetch = vi.fn(async () =>
      jsonResponse({
        loggedIn: true,
        profile: 'prod',
  ...
  it('keeps the cloud sign-in CTA visible after detection settles without surfacing AMR', async () => {
    ...
    expect(
      await screen.findByRole('button', { name: /Sign in to ComposerDesign/i }),
    ).toBeTruthy();
    expect(screen.queryByRole('button', { name: /ComposerDesign AMR/i })).toBeNull();
    expect(document.querySelector('.onboarding-view__card--skeleton')).toBeNull();
  });
