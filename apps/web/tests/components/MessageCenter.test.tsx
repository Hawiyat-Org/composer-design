...
const defaultMessages: MessageCenterMessage[] = [
  { id: 'release', audienceType: 'global', typeName: 'Product update', title: 'ComposerDesign 0.14 is available', body: 'The new release is ready.', ctaLabel: 'View update', ctaUrl: 'https://open-design.ai/update', publishedAt: '2026-07-16T12:00:00.000Z', readAt: null },
...
describe('MessageCenter', () => {
  ...
  it('renders API messages for anonymous clients without a local window', async () => {
    ...
    expect(within(dialog).getByText('ComposerDesign 0.14 is available')).toBeTruthy();
  ...
  it('keeps anonymous read state locally and restores it', async () => {
    renderMessageCenter();
    await openCenter();
    fireEvent.click(screen.getByRole('button', { name: /ComposerDesign 0\.14 is available/ }));
  ...
  it('expands the whole message row and opens its CTA', async () => {
    ...
    renderMessageCenter();
    await openCenter();
    const row = screen.getByRole('button', { name: /ComposerDesign 0\.14 is available/ });
  ...
  it('keeps visible account messages when locale changes and the follow-up sync fails', async () => {
    ...
    await openCenter();
    await waitFor(() => expect(screen.getByText('ComposerDesign 0.14 is available')).toBeTruthy());

    fireEvent.click(screen.getByRole('button', { name: 'Switch locale' }));

    await waitFor(() => expect(messageRequests).toBeGreaterThanOrEqual(2));
    expect(screen.getByText('ComposerDesign 0.14 is available')).toBeTruthy();
