...
interface Props {
  ...
  onComposerDesignSystem?: (id: string) => void;
...
export function EntryShell({
  ...
  const railWorkspaceContext = accountFooterState === 'sign-in'
    ? null
    : workspaceContext;
  const usesComposerDesignCloud = config.mode === 'daemon' && config.agentId === 'amr';
  ...
  useEffect(() => {
    ...
    const selectedCloudIdentityRejected = usesComposerDesignCloud && amrLoggedIn === false;
    if ((!selectedCloudIdentityRejected && !amrAuthRequired) || view === 'onboarding') return;
    navigate({ kind: 'home', view: 'onboarding' }, { replace: true });
  }, [amrAuthRequired, amrLoggedIn, usesComposerDesignCloud, view]);
  ...
  async function handlePluginLoopSubmit(payload: PluginLoopSubmit) {
    ...
    // ComposerDesign Cloud pre-run balance gate: hard blocks (empty wallet or
   ...
   * / Workspace 设置, and the "sign in to ComposerDesign Cloud" callout still in
              ...
              {designSystemsLoading ? (
                <div className="entry-section">
                  <DesignSystemsTab
                    ...
                    onOpenSystem={onComposerDesignSystem}
  ...
  async function pollAmrLoginCompletion(): Promise<boolean> {
    ...
    while (!amrLoginPollCancelledRef.current) {
      ...
      if (outcome === 'signed-in') {
        ...
        // shape (still showing the "sign in to ComposerDesign Cloud" callout)
      ...
      >
        <div className="onboarding-cloud__pane">
          ...
          <footer className="onboarding-cloud__footer">
            ...
            <span>
              © {new Date().getFullYear()} ComposerDesign · {t('settings.onboardingCloudRights')}
            </span>
          </footer>
        </div>
      ...
      >
        <div className={`onboarding-cloud__pane ${onboardingSourceStyles.pane}`}>
          ...
          <footer className="onboarding-cloud__footer">
            <LanguageMenu placement="up" align="start" />
            <span>
              © {new Date().getFullYear()} ComposerDesign ·{' '}
