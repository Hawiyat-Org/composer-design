...
// `contribute` opens a draft PR against the `nexu-io/composer-design` community
...
function buildContributePrompt(folderPath: string): string {
  return [
    'Open a draft Pull Request that adds this generated plugin to the ComposerDesign community catalog at `nexu-io/open-design`.',
    ...
    `Manifest: \`${folderPath}/open-design.json\``,
    '',
    'Run this deterministic ComposerDesign CLI workflow from the current project workspace:',
    '',
...
// ComposerDesign issue URL and belongs to the "ComposerDesign PR" button. Before
...
function buildPublishPrompt(folderPath: string): string {
  return [
    ...
    `Manifest: \`${folderPath}/open-design.json\``,
    '',
    'This is the **repository publish** action, NOT the registry-submission action — do NOT route through `od plugin publish --to open-design`. That command emits an ComposerDesign issue URL and belongs to the "ComposerDesign PR" button.',
    '',
    'Run this deterministic ComposerDesign CLI workflow from the current project workspace:',
    '',
