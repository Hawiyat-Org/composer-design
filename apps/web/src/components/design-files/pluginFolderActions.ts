...
// `contribute` opens a draft PR against the `hawiyat/composer-design` community
...
function buildContributePrompt(folderPath: string): string {
  return [
    'Open a draft Pull Request that adds this generated plugin to the Composer Design community catalog at `hawiyat/composer-design`.',
    ...
    `Manifest: \`${folderPath}/open-design.json\``,
    '',
    'Run this deterministic Composer Design CLI workflow from the current project workspace:',
    '',
...
// Composer Design issue URL and belongs to the "Composer Design PR" button. Before
...
function buildPublishPrompt(folderPath: string): string {
  return [
    ...
    `Manifest: \`${folderPath}/open-design.json\``,
    '',
    'This is the **repository publish** action, NOT the registry-submission action — do NOT route through `od plugin publish --to open-design`. That command emits an Composer Design issue URL and belongs to the "Composer Design PR" button.',
    '',
    'Run this deterministic Composer Design CLI workflow from the current project workspace:',
    '',
