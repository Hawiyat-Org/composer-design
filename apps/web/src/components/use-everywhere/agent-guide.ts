...
// everything it needs to install ComposerDesign, expose it as MCP, and
...
export interface AgentGuideOptions {
   ...
   */
  cliHint?: string;
  /** Optional ComposerDesign version/channel; surfaced in the header for support tickets. */
  versionHint?: string;
}
...
export function buildAgentGuideMarkdown(options: AgentGuideOptions = {}): string {
  ...
  lines.push('# ComposerDesign — agent setup guide');
  lines.push('');
  lines.push(
    'You are a coding agent (Claude Code, Codex, Cursor, openclaw, hermes, or similar). ' +
      'The user wants ComposerDesign wired into your workflow. ComposerDesign is a local ' +
  ...
  if (options.versionHint) {
    lines.push('');
    lines.push(`> Reported ComposerDesign version: \`${options.versionHint}\``);
  }
  lines.push('');
  ...
  lines.push('   ```');
  lines.push('');
  lines.push('   If it 404s or times out, ask the user to run `pnpm tools-dev` (dev) or open the ComposerDesign app (packaged).');
  lines.push('');
  if (installInfo) {
