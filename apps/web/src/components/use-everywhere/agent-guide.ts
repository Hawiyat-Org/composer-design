...
// everything it needs to install Composer Design, expose it as MCP, and
...
export interface AgentGuideOptions {
   ...
   */
  cliHint?: string;
  /** Optional Composer Design version/channel; surfaced in the header for support tickets. */
  versionHint?: string;
}
...
export function buildAgentGuideMarkdown(options: AgentGuideOptions = {}): string {
  ...
  lines.push('# Composer Design — agent setup guide');
  lines.push('');
  lines.push(
    'You are a coding agent (Claude Code, Codex, Cursor, openclaw, hermes, or similar). ' +
      'The user wants Composer Design wired into your workflow. Composer Design is a local ' +
  ...
  if (options.versionHint) {
    lines.push('');
    lines.push(`> Reported Composer Design version: \`${options.versionHint}\``);
  }
  lines.push('');
  ...
  lines.push('   ```');
  lines.push('');
  lines.push('   If it 404s or times out, ask the user to run `pnpm tools-dev` (dev) or open the Composer Design app (packaged).');
  lines.push('');
