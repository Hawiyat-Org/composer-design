...
export const ja: Dict = {
  ...
  'chat.amrCard.switchBody': 'Composer Design 公式の AMR モデルサービスに切り替えてください — API キーの設定は不要です。サインイン・認可・チャージが完了すると、このタスクは自動で再試行されます。',
  ...
  'chat.runError.signInMessage.amr': 'Composer Design エージェントはまだサインインしていません。サインインして使い始めてください。',
  'chat.runError.signInMessage.other': '{agent} がサインインしていません。ローカルでサインイン状態を確認してください。より安定してお得な Composer Design エージェントがおすすめです。',
  ...
  'settings.welcomeTitle': 'Composer Design へようこそ',
  'settings.welcomeSubtitle': '',
  'settings.onboardingCreateTitle': '概要から始める',
  'settings.onboardingCreateBody': '作成したいサイト、アプリ、スライド、画像、動画を記述してください。Composer Design がプロジェクトを作成し、編集可能な状態で作業を保持します。',
  ...
  'settings.onboardingMemoryCalloutBody': 'これらの回答はメモリのプロフィールの基礎になります。Composer Design はすべてのタスクでこれを再利用し、作業を進めるほど学習を続けます。',
  ...
  'settings.onboardingCloudTitle': 'Composer Design にサインイン',
  'settings.onboardingCloudBody': 'サインインすれば、クラウド AI ですぐにデザインを始められます。複雑な設定は一切不要です。',
  'settings.onboardingCloudSignIn': 'Composer Design Cloud にサインイン',
  ...
  'settings.onboardingGateTooltipAmr': 'この先のステップは AI で実行されます。続けるには Composer Design AMR にサインインしてください。',
  ...
  'settings.agentInstall.pathHint': 'npm または Homebrew で CLI をインストールしたのに未インストールのまま表示される場合は、Composer Design daemon が引き継ぐ PATH にツールの bin ディレクトリが含まれていることを確認してください（macOS では Terminal と GUI アプリで PATH が異なる場合があります）。QUICKSTART.md（"Local agent CLI and PATH" セクション）を参照してください。',
  'settings.agentInstall.stepOpenLinks': '利用したいエージェントの「インストール」または「ドキュメント」を開きます。',
  'settings.agentInstall.stepAuth': 'Composer Design に戻る前に、ベンダー CLI で認証（サインインまたは API 資格情報の追加）を行います。',
  ...
  'settings.mcpServerHint': 'Composer Design を MCP サーバーとしてコーディングエージェントに公開します。',
  ...
  'settings.privacyHint': 'Composer Design チームと共有されるデータ',
  'settings.privacyConsentKicker': 'Composer Design の改善にご協力ください',
  'settings.privacyConsentLead': 'Composer Design は、改善のために使用状況データをチームと共有できます。これには次の内容が含まれます:',
  ...
  'settings.mcpHint': 'コーディング Agent（Cursor、Claude Code、VS Code など）が Composer Design プロジェクトを直接読み取れるようにします。',
  'settings.mcpDaemonError': 'インストールパスを解決するためのローカルデーモンに接続できませんでした（{error}）。Composer Designが実行中であることを確認してから、このパネルを再度開いてください。',
  ...
  'settings.mcpCapabilityDefault': 'Composer Designで開いているプロジェクトとファイルをデフォルトにするため、どのデザインかを言い直すことなく「これを私のアプリで作って」と言えます。',
  'settings.mcpRunningNote': 'MCPツールの呼び出しを成功させるには、Composer Designが実行中である必要があります。Composer Designを開く前にコーディングAgentを起動した場合は、ライブデーモンに接続できるようAgentを再起動してください。',
  ...
  'entry.discordAria': 'Composer Design の Discord に参加',
  'entry.discordAriaWithOnline': 'Composer Design の Discord に参加 - {online}',
  ...
  'homeHero.chip.createPluginHint': '再利用可能な Composer Design プラグインを作成し、マイプラグインに追加します。',
  ...
  'handoff.promptIntro': 'このローカルのComposer Designプロジェクトフォルダから続行してください:',
  ...
  'pluginsHome.subtitle': 'このランタイムに同梱された、すぐに使える Composer Design ワークフロー。1つ選んでスタータープロンプトを読み込むか、レジストリでさらに探せます。',
  'pluginsHome.browseRegistry': 'レジストリを見る',
  'pluginsHome.count': '{total}件中{filtered}件',
  'pluginsHome.loadingCatalog': 'カタログを読み込み中…',
  'pluginsHome.emptyCatalog': 'カタログは空です。バンドルされたプラグインは Composer Design に同梱されており、ここに自動的に表示されるはずです — 解消しない場合はデーモンを再起動してみてください。',
  ...
  'pluginCard.contributeAria': '{title} を Composer Design に貢献',
  'pluginCard.contributeTitle': 'プラグインを pull request で Composer Design に貢献',
  ...
  'integrations.lede': '外部システムを接続し、MCP ツールをエージェントループに取り込み、他の IDE やスクリプト、自動化から Composer Design を利用できます。',
  ...
  'mcpClient.daemonError': 'ローカルデーモンに接続できませんでした。Composer Design が実行中であることを確認してから、このパネルを再度開いてください。',
  'mcpClient.saveFailed': '保存に失敗しました。デーモンが実行中であることを確認して、もう一度お試しください。',
  'useEverywhere.modalAria': 'Composer Design をあらゆる場所で使う',
  'useEverywhere.modalTitle': 'Composer Design をあらゆる場所で使う',
  'useEverywhere.modalSubtitle': 'Composer Design をあらゆる IDE、Agent、スクリプトに組み込めます — CLI、HTTP、MCP、Skills 対応。「Agent 用のガイドをコピー」を使って Claude Code、Codex、Cursor、openclaw、hermes に貼り付ければ、すべてをセットアップできます。',
  ...
  'useEverywhere.section.overview.heading': 'Composer Design は、お使いの Agent が動く場所ならどこでも動作します',
  'useEverywhere.section.overview.intro': 'Composer Design は単なるウィンドウ以上のものです — ローカルの特権デーモン（`od`）に加え、Skills + Design-Systems + Atoms のレジストリを備えています。マシン上で起動すれば、あらゆるコード Agent（Claude Code、Codex、Cursor、OpenCode/openclaw、Hermes、独自のスクリプト）が、4 つの相互に交換可能なインターフェースを通じて、生成の実行、プロジェクトの検査、デザイン成果物の作成を行えます。',
  'useEverywhere.section.overview.bullet1': 'CLI — ヘッドレスなスクリプト、CI、シェル自動化向けの `od <command>`。',
  'useEverywhere.section.overview.bullet2': 'MCP サーバー — Composer Design を Model Context Protocol サーバーとして接続し、MCP 対応のあらゆる Agent がスキルの一覧表示、シナリオの実行、成果物の読み取りを行えるようにします。',
  'useEverywhere.section.overview.bullet3': 'HTTP API — `http://127.0.0.1:7456/api/*` の REST + SSE エンドポイント。Web UI が使うのと同じインターフェースです。',
  'useEverywhere.section.overview.bullet4': 'Skills — ドロップインで使える `SKILL.md` パック（Claude 互換）。PATH 上にあるあらゆる Agent が、Composer Design なしでも呼び出せます。',
  ...
  'useEverywhere.section.cli.heading': '任意のシェルから Composer Design を操作する',
  ...
  'useEverywhere.section.mcp.heading': 'Composer Design を MCP サーバーとして任意のコーディング Agent に公開する',
  'useEverywhere.section.mcp.intro': 'Composer Design には Model Context Protocol サーバー（`od mcp`）が同梱されており、MCP 対応のあらゆるクライアント — Cursor、Claude Code、Antigravity、VS Code Copilot Chat、openclaw、hermes — が、手動でシェルを呼び出すことなく Composer Design のツール（スキルの一覧表示、プレビューのレンダリング、メディアの生成、プラグインの実行）を検出できます。デーモンは主要な各クライアント向けに、貼り付けるだけで使えるインストール用スニペットを `GET /api/mcp/install-info` で公開します。',
  ...
  'useEverywhere.section.mcp.footer': 'Composer Design アプリで「設定 → 連携」を開くと、JSON を手作業で編集する代わりに、クライアント別のインストールコマンド（Cursor、Claude Code、Antigravity、VS Code）をコピーできます。',
  ...
  'useEverywhere.section.skills.heading': 'あらゆる Agent 向けのドロップイン Skills — Composer Design が起動していなくても利用可能',
  'useEverywhere.section.skills.intro': 'Skill は、Claude 互換の `SKILL.md`（YAML フロントマター + 本文）を含むディレクトリです。Composer Design はこのフォーマットを `od:` 名前空間（`mode`、`preview`、`design_system`、`inputs`、…）で拡張し、同じ成果物を Composer Design 内でも、Claude Code、Codex、openclaw、hermes のような素の Agent でも利用できるようにしています。検出は優先順位のチェーンに従うため、プロジェクトは自身のスキルを上書きできます。',
  ...
  'useEverywhere.section.skills.snippet1': '最小限の SKILL.md（Claude 互換のフロントマター + Composer Design の追加項目）',
  ...
  'entry.githubStarAria': 'GitHub で Composer Design にスターを付ける',
  ...
  'newproj.locationDefault': 'Composer Design プロジェクト',
  ...
  'socialShare.openDesignSection': 'Composer Design を共有',
  'socialShare.openDesignTitle': 'GitHub の Composer Design',
  'socialShare.openDesignText': 'Composer Design は、デザイン成果物の作成・編集・デプロイ・引き継ぎを行うためのオープンソースのワークスペースです。',
  ...
  'socialShare.projectText': 'Composer Design で作成: {title}。デプロイされた成果物を見て、リポジトリにスターを付けてください: {repo}',
  'socialShare.projectCopyText': 'Composer Design で作成: {title}\n{url}\nComposer Design リポジトリ: {repo}',
  ...
  'chat.activeFilePlaceholder': 'Composer Design に {file} の変更を依頼...',
  ...
  'chat.mode.design.solves': 'Composer Design にファイル生成、workspace 更新、brief からマルチモーダルな成果物への変換を任せたいときに使います。',
  ...
  'preview.shareTextDefault': 'Composer Design テンプレート: {title}',
  ...
  'preview.errorBody': '例の HTML を取得できませんでした。Composer Design が起動していることを確認して再試行してください。',
  'preview.retry': '再試行',
  ...
  'assistant.shareToOpenDesign': 'Composer Design に共有',
  ...
  'pet.tabBuiltIn': '組み込み',
  'pet.tabBuiltInHint': 'Composer Design に同梱された厳選コンパニオン — 選んで迎え入れよう。',
  ...
  'pet.codexBundled': '同梱',
  'pet.codexBundledTitle': 'Composer Design に同梱 — ダウンロード不要。',
  ...
  'settings.projectLocationsDescription': '複数の Composer Design プロジェクトフォルダを含められる作業ベースを追加します。新しいプロジェクトは選択したベース内の 1 つのフォルダとして保存されます。',
  ...
  'settings.memoryStarterProjectDesc': 'Composer Design v0.5 — チャット駆動のデザインエディター',
  ...
  'updater.availableBody': 'Composer Design {version} を利用できます。インストーラーを開く前にダウンロードします。',
  ...
  'updater.installerOpenBody': 'インストーラーが開いています。更新を完了できるよう Composer Design を終了しています。',
  ...
  'updater.quitButton': 'Composer Design を終了',
  'updater.quitFailedBody': 'インストーラーは開いていますが、Composer Design を終了できませんでした。アプリを置き換える前に Composer Design を終了してください。',
  'updater.quitFailedTitle': '終了できませんでした',
  'updater.quitting': '終了しています...',
  'updater.ready': 'アップデート準備完了',
  'updater.readyGeneric': '新しいバージョンの準備ができました。Composer Design を閉じてインストーラーを開きます。',
  'updater.readyVersion': 'Composer Design {version} の準備ができました。Composer Design を閉じてインストーラーを開きます。',
  ...
  'dsManager.createBody': 'ブランド・プロダクト・コード・アセット・デザインの参考資料を Composer Design に学習させます。',
  ...
  'settings.customInstructionsDesc': 'Composer Design がすべてのチャットで従う固定の指示です。これらは保存されたメモリではありません。事実・好み・プロジェクトのコンテキストには「メモリ」を使用してください。',
