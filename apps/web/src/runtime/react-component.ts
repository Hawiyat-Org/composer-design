    ...
    <script>
      (function(){
        ...
        try {
          ...
          (0, eval)(compiled);
          var Component = window.__ComposerDesignComponent ||
...
window.__ComposerDesignComponent = window.__ComposerDesignComponent || (${componentFallbackExpression(transformed.defaultName)});`;
}
...
function transformExports(source: string): { code: string; defaultName: string | null } {
  ...
  code = code.replace(
    ...
    (_match, name: string | undefined) => {
      defaultName = name || 'ComposerDesignComponent';
  ...
  code = code.replace(/export\s+default\s+/g, () => {
    defaultName = 'ComposerDesignComponent';
    return 'const ComposerDesignComponent = ';
  });
  code = code.replace(
