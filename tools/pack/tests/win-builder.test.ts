...
function createPaths(root: string): WinPaths {
  ...
  return {
    ...
    blockmapPath: join(namespaceRoot, "builder", "Composer Design-second-setup.exe.blockmap"),
    ...
    exePath: join(namespaceRoot, "builder", "Composer Design-second.exe"),
    installDir: join(namespaceRoot, "runtime", "install", "Composer Design"),
    installedExePath: join(namespaceRoot, "runtime", "install", "Composer Design", "Composer Design.exe"),
    ...
    launcherPayloadPath: join(namespaceRoot, "payload", "Composer Design-second-payload.7z"),
    ...
    setupPath: join(namespaceRoot, "builder", "Composer Design-second-setup.exe"),
    setupZipPath: join(namespaceRoot, "builder", "Composer Design-second-portable.zip"),
    ...
    uninstallerPath: join(namespaceRoot, "runtime", "install", "Composer Design", "Uninstall.exe"),
...
describe("materializeCachedUnpackedForInstaller", () => {
  it("overwrites cached packaged config and app package version", async () => {
    const root = await mkdtemp(join(tmpdir(), "composer-design-win-builder-"));
    ...
    try {
      ...
      await writeFile(join(cachedUnpackedRoot, "Composer Design.exe"), await createVersionedExecutable("0.5.0-beta.1"));
      await writeFile(
      ...
      await expect(readWinExecutableVersionSnapshot(join(paths.unpackedRoot, "Composer Design.exe"))).resolves.toMatchObject({
...
async function createVersionedExecutable(packagedVersion: string): Promise<Buffer> {
  ...
  version.setStringValues(
    ...
    {
      FileDescription: "Composer Design",
