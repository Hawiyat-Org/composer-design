...
describe("resolveWinInstallIdentity", () => {
  it("keeps the default namespace on the canonical Windows display name", () => {
    expect(resolveWinInstallIdentity({ namespace: "default" })).toMatchObject({
      displayName: "Composer Design",
      shortcutName: "Composer Design.lnk",
      uninstallerName: "Uninstall Composer Design.exe",
    });
  });
  ...
  it("uses the canonical Windows display name for stable release namespaces", () => {
    expect(resolveWinInstallIdentity({ namespace: "release-stable-win" })).toMatchObject({
      appPathsKey: "Software\\Microsoft\\Windows\\CurrentVersion\\App Paths\\Composer Design.exe",
      displayName: "Composer Design",
      registryKey: "Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\Composer Design-release-stable-win",
      shortcutName: "Composer Design.lnk",
      uninstallerName: "Uninstall Composer Design.exe",
    });
  });
  ...
  it("uses first-class beta display identity for beta release namespaces", () => {
    expect(resolveWinInstallIdentity({ namespace: "release-beta-win" })).toMatchObject({
      appPathsKey: "Software\\Microsoft\\Windows\\CurrentVersion\\App Paths\\Composer Design Beta.exe",
      displayName: "Composer Design Beta",
      registryKey: "Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\Composer Design-release-beta-win",
      shortcutName: "Composer Design Beta.lnk",
      uninstallerName: "Uninstall Composer Design Beta.exe",
    });
  });
  ...
  it("keeps non-release beta-like namespaces isolated from the real beta channel identity", () => {
    expect(resolveWinInstallIdentity({ namespace: "beta-local-flow" })).toMatchObject({
      appPathsKey: "Software\\Microsoft\\Windows\\CurrentVersion\\App Paths\\Composer Design beta-local-flow.exe",
      displayName: "Composer Design beta-local-flow",
      registryKey: "Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\Composer Design-beta-local-flow",
      shortcutName: "Composer Design beta-local-flow.lnk",
      uninstallerName: "Uninstall Composer Design beta-local-flow.exe",
    });
  });
  ...
  it("uses first-class preview display identity for preview release namespaces", () => {
    expect(resolveWinInstallIdentity({ namespace: "release-preview-win" })).toMatchObject({
      appPathsKey: "Software\\Microsoft\\Windows\\CurrentVersion\\App Paths\\Composer Design Preview.exe",
      displayName: "Composer Design Preview",
      registryKey: "Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\Composer Design-release-preview-win",
      shortcutName: "Composer Design Preview.lnk",
      uninstallerName: "Uninstall Composer Design Preview.exe",
    });
  });
  ...
  it("uses first-class prerelease display identity for prerelease release versions and namespaces", () => {
    expect(resolveWinInstallIdentity({
      ...
      appPathsKey: "Software\\Microsoft\\Windows\\CurrentVersion\\App Paths\\Composer Design Prerelease.exe",
      displayName: "Composer Design Prerelease",
      registryKey: "Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\Composer Design-release-stable-win",
      shortcutName: "Composer Design Prerelease.lnk",
      uninstallerName: "Uninstall Composer Design Prerelease.exe",
    });
  ...
  it("emits a valid NSIS command literal for executable paths containing spaces", () => {
    expect(createNsisQuotedCommandLiteral(["$INSTDIR\\Composer Design.exe", "%1"])).toBe(
      `'"$INSTDIR\\Composer Design.exe" "%1"'`,
    );
    expect(createNsisQuotedCommandLiteral(["$INSTDIR\\Composer Design.exe"])).toBe(
      `'"$INSTDIR\\Composer Design.exe"'`,
    );
  });
