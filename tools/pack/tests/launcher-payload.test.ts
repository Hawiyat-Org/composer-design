...
async function writeFakeMacApp(config: ToolPackConfig): Promise<ReturnType<typeof resolveMacPaths>> {
  ...
  await mkdir(join(resourcesRoot, "composer-design", "prebundled", "daemon"), { recursive: true });
  await mkdir(join(resourcesRoot, "composer-design", "prebundled", "web"), { recursive: true });
  await mkdir(join(resourcesRoot, "composer-design-web-standalone"), { recursive: true });
  await writeFile(executablePath, "#!/bin/sh\nexit 0\n", "utf8");
  await chmod(executablePath, 0o755);
  await writeFile(join(resourcesRoot, "composer-design", "bin", "node"), "#!/bin/sh\nexit 0\n", "utf8");
  await writeFile(join(resourcesRoot, "composer-design", "prebundled", "daemon", "daemon-sidecar.mjs"), "export {};\n", "utf8");
  await writeFile(join(resourcesRoot, "composer-design", "prebundled", "web", "web-sidecar.mjs"), "export {};\n", "utf8");
  await writeFile(
    join(resourcesRoot, "composer-design-config.json"),
    `${JSON.stringify({
      appVersion: config.appVersion,
      daemonSidecarEntryRelative: "composer-design/prebundled/daemon/daemon-sidecar.mjs",
      namespace: config.namespace,
      nodeCommandRelative: "composer-design/bin/node",
      webOutputMode: "standalone",
      webSidecarEntryRelative: "composer-design/prebundled/web/web-sidecar.mjs",
    }, null, 2)}\n`,
    "utf8",
  );
  return paths;
...
function createWinPaths(root: string, namespace: string): WinPaths {
  ...
  return {
    ...
    blockmapPath: join(namespaceRoot, "builder", "Composer Design-release-beta-win-setup.exe.blockmap"),
    ...
    exePath: join(namespaceRoot, "builder", "Composer Design-release-beta-win.exe"),
    installDir: join(namespaceRoot, "runtime", "install", "Composer Design Beta"),
    installedExePath: join(namespaceRoot, "runtime", "install", "Composer Design Beta", "Composer Design.exe"),
    ...
    launcherPayloadPath: join(namespaceRoot, "payload", "Composer Design-release-beta-win-payload.7z"),
    ...
    packagedConfigPath: join(namespaceRoot, "composer-design-config.json"),
    ...
    setupPath: join(namespaceRoot, "builder", "Composer Design-release-beta-win-setup.exe"),
    setupZipPath: join(namespaceRoot, "builder", "Composer Design-release-beta-win-portable.zip"),
    ...
    uninstallerPath: join(namespaceRoot, "runtime", "install", "Composer Design Beta", "Uninstall.exe"),
    ...
    unpackedExePath: join(namespaceRoot, "builder", "win-unpacked", "Composer Design.exe"),
...
async function writeFakeWinUnpackedApp(root: string, namespace: string, version: string): Promise<{
  ...
  await writeFile(join(paths.unpackedRoot, "Composer Design.exe"), "fake executable\n", "utf8");
  await writeFile(
    join(paths.unpackedRoot, "resources", "composer-design-config.json"),
    `${JSON.stringify({
      appVersion: version,
      daemonSidecarEntryRelative: "composer-design/prebundled/daemon/daemon-sidecar.mjs",
      namespace,
  ...
  await writeFile(
    ...
    `${JSON.stringify({ name: "composer-design-packaged-app", version })}\n`,
    "utf8",
  );
  ...
  return {
    builtApp: {
      ...
      executablePath: join(paths.unpackedRoot, "Composer Design.exe"),
      source: "namespace",
...
describe("tools-pack launcher payload archives", () => {
  it("builds channel and namespace scoped payload manifests for both desktop platforms", () => {
    ...
    expect(buildMacLauncherPayloadManifest({
      ...
      appBundleName: "Composer Design Beta.app",
      channel: "beta",
      entry: {
  ...
  it.skipIf(process.platform !== "darwin")("creates a mac payload zip with bootstrap-readable contents", async () => {
    ...
    try {
      ...
      expect(manifest.appBundleName).toBe("Composer Design Beta.app");
      expect(manifest.entry.executable).toBe("payload/Composer Design Beta.app/Contents/MacOS/Composer Design Beta");
      ...
      await expectPathExists(join(
        extractRoot,
        "payload",
        "Composer Design Beta.app",
        "Contents",
        "Resources",
        "composer-design-config.json",
      ));
    } finally {
  ...
  it.skipIf(process.platform !== "win32")("creates a Windows payload 7z with bootstrap-readable contents", async () => {
    ...
    try {
      ...
      expect(manifest.entry.executable).toBe("payload/Composer Design.exe");
      expect(manifest.version).toBe(version);
      await expectPathExists(join(extractRoot, "payload", "Composer Design.exe"));
      await expectPathExists(join(extractRoot, "payload", "resources", "composer-design-config.json"));
    } finally {
      await rm(root, { force: true, recursive: true });
    }
  });
  ...
  it.skipIf(process.platform !== "win32")("reuses the Windows payload archive when base and overlay metadata are unchanged", async () => {
    ...
    try {
      ...
      const config = JSON.parse(
        await readFile(join(extractRoot, "payload", "resources", "composer-design-config.json"), "utf8"),
